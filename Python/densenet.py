# -*- coding: utf-8 -*-
"""
Created on Fri Dec 13 11:49:15 2019
...
class TestModel(tf.keras.Model):
    ...
    def __init__(self, num_blocks=1, hidden_size=32, growth_rate=8, rate=0.1, l2=1e-3, activation="relu", loss=custom_loss_breslow, learning_rate=1e-4):
        super(TestModel, self).__init__()

        self.dropout = tf.keras.layers.Dropout(rate)  ###
        self.initial_layer = tf.keras.layers.Dense(hidden_size, activation=activation,
                                                   kernel_regularizer=tf.keras.regularizers.L1L2(l1=0.01, l2=l2))

        # self._blocks = tf.keras.Sequential()
        self._blocks = []
        for i in range(num_blocks):
            self._blocks.append([tf.keras.layers.GaussianNoise(0.1),
                                 tf.keras.layers.Dropout(rate),
                                 tf.keras.layers.Dense(hidden_size, activation=activation,
                                                       kernel_regularizer=tf.keras.regularizers.L1L2(l1=0.01, l2=l2)),
                                 tf.keras.layers.GaussianNoise(0.1),
                                 tf.keras.layers.Dropout(rate),
                                 tf.keras.layers.Dense(growth_rate, activation=activation,
                                                       kernel_regularizer=tf.keras.regularizers.L1L2(l1=0.01, l2=l2))
                                 ])

        self.final_layer0 = tf.keras.layers.Dense(hidden_size, activation=activation,
                                                  kernel_regularizer=tf.keras.regularizers.L1L2(l1=0.01, l2=l2))  ####
        self.final_layer1 = tf.keras.layers.Dense(1, kernel_regularizer=tf.keras.regularizers.L1L2(l1=0.01, l2=l2))  ####
        self.loss_fn = loss
    ...
    def call(self, x, training=None):
        ...
        for layer in self._blocks:
            out1 = layer[0](out, training=training)
            out1 = layer[1](out1, training=training)
            out1 = layer[2](out1)

            out1 = layer[3](out1, training=training)
            out1 = layer[4](out1, training=training)
            out1 = layer[5](out1)

            out = tf.concat([out, out1], axis=-1)

        out = self.final_layer0(out)
...
dl_metrics = pd.DataFrame(columns=("imp","cidx_uno_test", "cidx_uno_lower","cidx_uno_upper"))
...
for i in range(1,41):
    ...
    for j in range(100):
        ...
        model.train(ds_train, ds_val, max_epochs=500, verbose=0)
